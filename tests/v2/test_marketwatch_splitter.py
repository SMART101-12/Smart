...
def test_ins_code_is_the_only_path_identity(tmp_path: Path):
    ...
    assert written[0].as_posix().endswith("123/2026-08/2026-08-19.json")
    assert "آزمایشی" not in written[0].as_posix()
    assert "TEST" not in written[0].as_posix()
...
def test_sparse_cells_are_aligned_by_excel_reference(tmp_path: Path):
    ...
    assert len(written) == 1
    assert written[0].as_posix().endswith("456/2026-08/2026-08-19.json")
    payload = json.loads(written[0].read_text(encoding="utf-8"))
    assert payload["instrument"]["symbol_fa"] == "فولاد"
    assert payload["instrument"]["ins_code"] == "456"
...
def test_header_can_appear_after_metadata_rows(tmp_path: Path):
    ...
    assert len(written) == 1
    assert written[0].as_posix().endswith("789/2026-08/2026-08-19.json")
    payload = json.loads(written[0].read_text(encoding="utf-8"))
    assert payload["instrument"]["symbol_fa"] == "آزمایش۲"
    assert payload["instrument"]["symbol_en"] == "TEST2"
...
def test_shared_strings_and_duplicate_ins_code(tmp_path: Path):
    ...
    assert len(written) == 1
    assert written[0].as_posix().endswith("999/2026-08/2026-08-19.json")
    payload = json.loads(written[0].read_text(encoding="utf-8"))
    assert payload["instrument"]["ins_code"] == "999"
    assert payload["instrument"]["symbol_fa"] == "نمونه"


def test_duplicate_ins_code_with_different_symbols_is_not_split_by_symbol(tmp_path: Path):
    xlsx = _xlsx_bytes(f'''<worksheet xmlns="{NS}"><sheetData>
        <row><c r="A1" t="inlineStr"><is><t>InsCode</t></is></c><c r="B1" t="inlineStr"><is><t>SymbolFa</t></is></c></row>
        <row><c r="A2"><v>999</v></c><c r="B2" t="inlineStr"><is><t>نمونه</t></is></c></row>
        <row><c r="A3"><v>999</v></c><c r="B3" t="inlineStr"><is><t>نمونه-قدیمی</t></is></c></row>
    </sheetData></worksheet>''')
    source, output = _write_snapshot(tmp_path, xlsx)
...
def test_missing_ins_code_is_not_written(tmp_path: Path):
    xlsx = _xlsx_bytes(f'''<worksheet xmlns="{NS}"><sheetData>
        <row><c r="A1" t="inlineStr"><is><t>SymbolFa</t></is></c></row>
        <row><c r="A2" t="inlineStr"><is><t>پالایش</t></is></c></row>
    </sheetData></worksheet>''')
    source, output = _write_snapshot(tmp_path, xlsx)
...
def test_real_marketwatch_persian_header_shape(tmp_path: Path):
    xlsx = _xlsx_bytes(f'''<worksheet xmlns="{NS}"><sheetData>
        ...
        <row r="3"><c r="A3" t="inlineStr"><is><t>InsCode</t></is></c><c r="B3" t="inlineStr"><is><t>نماد</t></is></c><c r="C3" t="inlineStr"><is><t>نام</t></is></c><c r="D3" t="inlineStr"><is><t>تعداد</t></is></c><c r="E3" t="inlineStr"><is><t>حجم</t></is></c></row>
        <row r="4"><c r="A4"><v>456</v></c><c r="B4" t="inlineStr"><is><t>فولاد</t></is></c><c r="C4" t="inlineStr"><is><t>فولاد مبارکه اصفهان</t></is></c><c r="D4"><v>100</v></c><c r="E4"><v>2000</v></c></row>
    </sheetData></worksheet>''')
    source, output = _write_snapshot(tmp_path, xlsx)
