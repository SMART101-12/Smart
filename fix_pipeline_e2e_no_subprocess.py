# -*- coding: utf-8 -*-
"""Generate and run a robust E2E pytest module for the Smart-Git v2 pipeline.

Windows-compatible, self-contained, NO subprocess import or call.
Copy to repo root (D:\\Smart-Git) and run:  python fix_pipeline_e2e_no_subprocess.py
"""
...
# ---------------------------------------------------------------- imports ---
from smart_v2.analysis.service import SmartMoneyAnalyzer  # noqa: E402
...
def _call_analyze(cls, df, aliases=("symbol", "ticker", "name"), sym="فولاد"):
...
def test_multi_factor_and_gold_fund_e2e():
    ...
    required = [p for p in sig.parameters.values()
                if p.default is inspect.Parameter.empty]
    if _find_param(sig, ("scores", "factors", "inputs", "data", "results")) \
            is not None or len(required) >= 4:
...
if __name__ == "__main__":
    # Self compile-check before writing anything
    compile(TEST_SOURCE, "test_pipeline_e2e.py", "exec")  # raises SyntaxError if bad

    target = REPO_ROOT / "tests" / "v2" / "test_pipeline_e2e.py"
    ...
    def main():
        return pytest.main(["tests/v2/test_pipeline_e2e.py", "-v"])
