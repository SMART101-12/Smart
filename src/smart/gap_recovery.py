...
ROOT=Path(__file__).resolve().parents[2]; DB_PATH=ROOT/"data"/"smart.db"; RUNTIME=ROOT/"runtime"; HISTORY_DIR=RUNTIME/"history"; QUALITY_DIR=RUNTIME/"data_quality"; RETRY_SECONDS=int(os.getenv("SMART_GAP_RETRY_SECONDS","3600")); MAX_ATTEMPTS=int(os.getenv("SMART_GAP_MAX_ATTEMPTS","0"))
...
def repair_symbol(symbol,*,today=None):
 ...
 start=min([*existing,*db_parsed]); q=_load_quality(symbol); dates=q.setdefault("dates",{}); missing=[d for d in _expected_dates(start,today) if d not in existing and d not in db_parsed]
 adapter=TsetmcAdapter(); ins_code=str(payload.get("ins_code","")).strip()
 if not ins_code:
  try: ins_code=str(adapter.resolve_symbol(symbol)["insCode"]); payload["ins_code"]=ins_code
  except Exception as exc:
   result={"symbol":symbol,"status":"symbol_resolve_failed","missing":[d.strftime("%Y%m%d") for d in missing],"error":str(exc)}; _save_quality(symbol,result); return result
 source_rows=adapter.daily_history(ins_code,0); source_by_date={d:r for r in source_rows if (d:=_date(r.get("dEven")))}; repaired=[]; unresolved=[]
 for d in missing:
  key=d.strftime("%Y%m%d"); e=dates.setdefault(key,{"attempts":0}); e["attempts"]=int(e.get("attempts",0))+1; e["last_check"]=datetime.now(timezone.utc).isoformat(); row=source_by_date.get(d)
  if row: existing[d]=row; e.update({"status":"DATA_AVAILABLE","market_open":True,"retry":False}); repaired.append(key)
  else: e.update({"status":"UNRESOLVED","market_open":"unknown","retry":True}); unresolved.append(key)
 payload["daily_history"]=list(existing.values()); _save_history(symbol,payload); q["summary"]={"missing_before":len(missing),"repaired":len(repaired),"unresolved":len(unresolved),"next_retry_seconds":RETRY_SECONDS}; _save_quality(symbol,q); _sync_to_git(symbol,payload,q); return {"symbol":symbol,**q["summary"],"repaired_dates":repaired,"unresolved_dates":unresolved}
def run(symbols):
 while True:
