...
def repair_symbol(symbol: str, *, today: date | None = None, dry_run: bool = False) -> dict[str, Any]:
    ...
    calendar_dates = _calendar_dates(adapter, ins_code); scan_end = today - timedelta(days=1)
    candidate_dates = set(_candidate_week_dates(start, scan_end)); expected_dates = {d for d in candidate_dates if d in calendar_dates}
    ...
    for d in sorted(expected_dates):
        ...
        try:
            ...
            if isinstance(row, dict) and _date(row.get("dEven")) == d:
                existing[d] = row; dates.pop(key, None); repaired.append(key)
            else:
                entry.update({"status": "UNRESOLVED", "market_open": True, "retry": True}); unresolved.append(key)
        except Exception as exc:
            entry.update({"status": "FETCH_FAILED", "market_open": True, "retry": True, "error": str(exc)}); unresolved.append(key)
    payload["daily_history"] = list(existing.values())
    result = {"symbol": symbol, "calendar_dates": len(calendar_dates), "expected_trading_dates": len(expected_dates), "stale_false_positive_dates_removed": len(stale_keys), "repaired": len(repaired), "unresolved": len(unresolved), "repaired_dates": repaired, "unresolved_dates": unresolved, "removed_false_positive_dates": stale_keys, "dry_run": dry_run}
    if not dry_run:
        _save_monthly(symbol, payload); quality["summary"] = result; _save_quality(symbol, quality); _sync_to_git(symbol, payload, quality)
    return result
