...
def main() -> int:
    ...
    ins_idx = (
        normalized.get("inscode")
    )

    symbol_idx = (
        normalized.get("symbolfa")
        or normalized.get("symbol")
        or normalized.get("l18")
        or normalized.get("lval18")
    )
    ...
    for row in rows[header_pos + 1:]:
        ...
        if (
            symbol_idx is not None
            and symbol_idx < len(row)
        ):
    ...
    duplicates = (
        nonempty - len(set(ins_codes))
    )


    print(
        f"DATA_ROWS_WITH_INSCODE={nonempty}"
    )

    print(
        f"UNIQUE_INSCODE={len(set(ins_codes))}"
    )

    print(
        f"DUPLICATE_INSCODE={duplicates}"
    )

    print(
        f"NONASCII_SYMBOLS={nonascii_symbols}"
    )
