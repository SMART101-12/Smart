...
def signal(name, rows):
    base, p = name.rsplit("0",1) if name.endswith(tuple(str(i) for i in range(10))) else (name, "20")
    # robust parser because periods may be 10/20/30 etc.
    for q in sorted(PERIODS, key=len, reverse=True):
    ...
    if base == "sma": return clip((last/sma(c,p)-1)*10) if sma(c,p) else 0
    if base == "ema": return clip((last/ema(c,p)-1)*10) if ema(c,p) else 0
    if base == "momentum": return clip((last/c[-p]-1)*4) if len(c)>p else 0
    if base == "roc": return clip((last/c[-p]-1)*6) if len(c)>p else 0
    if base == "rsi": return clip((rsi(c,p)-50)/25) if rsi(c,p) is not None else 0
    if base == "stoch":
    ...
    if base == "obv":
        ...
        s=slope(vals,p); return clip(s/(avg([abs(z) for z in vals[-p:]])+1e-9)*20) if s is not None else 0
    if base == "mfi":
        pos=neg=0
    ...
    if base in {"ichimoku","tenkan_kijun","cloud_thickness","price_cloud"}:
        t=max(h[-max(2,p//2):]); tl=min(l[-max(2,p//2):]); ten=(t+tl)/2
        k=max(h[-p:]); kl=min(l[-p:]); kij=(k+kl)/2
        spanb=(max(h[-p*2:])+min(l[-p*2:]))/2 if len(c)>=p*2 else kij
        spana=(ten+kij)/2
        if base=="ichimoku": return clip(((last-(spana+spanb)/2)/last)*15 + (1 if ten>kij else -1)*.25)
...
def forecast(feature_set, rows):
    ...
    score=avg(scores) or 0
    c=close(rows)
    # volatility-aware forecast; the scale is learned only from the past window.
    vol=std([close(r) for r in rows],20)
...
def eval_model(feature_set, rows, start, end):
    ...
    for i in range(max(start,30),min(end,len(rows)-1)):
        hist=rows[:i+1]; pred=forecast(feature_set,hist); cur=close(hist); actual=close(rows[i+1]); naive=cur
        me=abs(pred/actual-1)*100; be=abs(naive/actual-1)*100
...
def model_catalog():
    # 200 point-in-time indicator variants: 20 families x 10 periods.
    # At least 120 first-stage models, then model-of-model combinations.
...
def main():
    rows=load_rows(); n=len(rows); train_end=int(n*.70); val_end=int(n*.85)
    catalog=model_catalog()
    first=[]
    for idx,fs in enumerate(catalog,1):
        tr=eval_model(fs,rows,30,train_end); va=eval_model(fs,rows,train_end,val_end)
        first.append({"model_id":f"M{idx:03d}","features":fs,"train":tr,"validation":va})
    first.sort(key=lambda x:(-x["validation"]["improvement_pct"],-x["validation"]["direction_pct"],x["validation"]["mae_pct"]))
    top=first[:15]
    second=[]
    for a,b in itertools.combinations(top,2):
        fs=("MODEL:"+a["model_id"],"MODEL:"+b["model_id"])
        # model-of-model forecast is the average of component forecasts.
        errs=[]; dirs=[]; beats=[]
        for i in range(max(30,train_end),val_end):
            hist=rows[:i+1]; pa=forecast(a["features"],hist); pb=forecast(b["features"],hist); pred=(pa+pb)/2; cur=close(hist); actual=close(rows[i+1]); naive=cur
            errs.append(abs(pred/actual-1)*100); dirs.append((pred>=cur)==(actual>=cur)); beats.append(abs(naive/actual-1)*100-errs[-1])
    ...
    winners=second[:10]
    oos=[]
    by={x["model_id"]:x for x in top}
    for m in winners:
        errs=[]; dirs=[]; beats=[]
        a,b=[by[x] for x in m["parents"]]
        for i in range(val_end,n-1):
            hist=rows[:i+1]; pred=(forecast(a["features"],hist)+forecast(b["features"],hist))/2; cur=close(hist); actual=close(rows[i+1]); ne=abs(cur/actual-1)*100; me=abs(pred/actual-1)*100
            errs.append(me); beats.append(ne-me); dirs.append((pred>=cur)==(actual>=cur))
    ...
    result={"engine_version":"ensemble-v2.0","symbol":SYMBOL,"rows":n,"first_date":rows[0]["dEven"],"last_date":rows[-1]["dEven"],"feature_variants":len(FEATURES),"first_stage_models":len(catalog),"second_stage_models":len(second),"train_end_index":train_end,"validation_end_index":val_end,"no_lookahead":True,"learning":"model selection uses train/validation only; frozen OOS is never used for tuning","top_first_stage":top,"top_model_of_models":winners,"frozen_oos":oos,"final_candidate":oos[0] if oos else None}
